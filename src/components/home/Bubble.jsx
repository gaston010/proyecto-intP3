...
const Bubble = ({data}) => {
    ...
    useEffect(() => { //Se ejecuta una única vez al primer renderizado
        let interval;

        if(initial){
            setTimeout(() => {
                interval = setInterval(() => setTransition(transition === 0 ? 1 : 0), 2000);
            }, data.delay);
            setInitial(false);
        } else {
            interval = setInterval(() => setTransition(transition === 0 ? 1 : 0), 2000);
  ...
  return (
    <div className={
        `rounded-full size-48 shadow-lg shadow-fuchsia-500 transition ease-in-out
